...
def sort_menu():
    ...
    sorted_menu = df_sorted.groupby('Category')['Item'].apply(list).to_dict()
...
def recommend():
   ...
   # drink_vector = vectorizer.transform([drink_name])
    ...
    recommended_foods = process.extractBests(food_name, food_df['Item'].tolist(), limit=100)
    recommended_drinks = process.extractBests(drink_name, drink_df['Item'].tolist(), limit=100)

    # Extract the item names and their categories from the recommendations
    recommended_foods = [{"name": food[0], "category": food_df[food_df['Item'] == food[0]]['Category'].values[0]} for food in recommended_foods]
    recommended_drinks = [{"name": drink[0], "category": drink_df[drink_df['Item'] == drink[0]]['Category'].values[0]} for drink in recommended_drinks]
    ...
    return jsonify({
       ...
       #'recommended_foods': [food[0] for food in recommended_foods],
        #'recommended_drinks': [drink[0] for drink in recommended_drinks],
    })

@app.route('/api/menu', methods=['GET'])
def get_menu():
    # Create a dictionary to hold menu items for each category
    menu = {'Foods': [], 'Drinks': []}
    ...
    for index, row in df.iterrows():
        ...
        if category in ['Beverages', 'Coffee & Tea']:
            menu['Drinks'].append(item)
        else:
            menu['Foods'].append(item)

    # Return the menu items for each category as a JSON response
    return jsonify(menu)
